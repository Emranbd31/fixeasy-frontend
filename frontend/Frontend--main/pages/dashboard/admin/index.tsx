...
const metrics = [
  { label: 'Total users', value: '12,482', accent: 'brand' as const, icon: <Users className="h-6 w-6" /> },
  { label: 'Verified pros', value: '518', accent: 'emerald' as const, icon: <ShieldCheck className="h-6 w-6" /> },
  { label: 'Pending pros', value: '42', accent: 'amber' as const, icon: <ClipboardList className="h-6 w-6" /> },
  { label: 'Active bookings', value: '327', accent: 'brand' as const, icon: <Activity className="h-6 w-6" /> },
]
...
export default function AdminDashboardPage() {
  const [health, setHealth] = useState<HealthStatus>({ status: 'unknown', message: 'Checking…' })
