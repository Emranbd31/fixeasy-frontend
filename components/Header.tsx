        ...
        <div className="mx-auto flex h-20 w-full max-w-6xl items-center justify-between px-5 sm:px-6 lg:px-8">
          ...
          <nav className="hidden items-center gap-6 text-sm font-medium text-white/75 lg:flex">
            {navItems.map((item) => (
              ...
              >
                {item.label}
                <span className="absolute -bottom-2 left-0 h-[2px] w-0 rounded-full bg-gradient-to-r from-cyan-300 via-white to-rose-300 transition-all duration-300 ease-smooth group-hover:w-full" />
              </Link>
            ))}
          </nav>
          <div className="flex items-center gap-3">
