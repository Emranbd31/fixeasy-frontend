...
export default function ClientDashboardPage() {
    ...
    return (
      <main className="p-10">
        <p>
          Please{' '}
          <Link
            className="text-blue-600 transition-transform duration-200 hover:scale-105 hover:shadow-md hover:underline"
            href="/login"
          >
            login
          </Link>{' '}
  ...
  const upcoming = bookings.filter((booking) => booking.status === 'Scheduled');
  ...
  const resolveNotificationVisuals = (type: Notification['type']) => {
    if (type === 'success') {
      return {
    ...
    return {
      icon: <Info className="w-4 h-4 text-blue-500 mr-2" />,
    ...
    <main className="min-h-screen bg-gradient-to-b from-white to-gray-50 py-12">
          ...
          <ul className="space-y-2">
            {notifications.map((notification) => {
              ...
              return (
                <li
                  key={notification.id}
                  className={`fade-in-notification flex items-center border-l-4 p-3 rounded text-sm ${visuals.styles}`}
                >
                  {visuals.icon}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mb-12">
          <div className="bg-white rounded-2xl shadow p-4 sm:p-6 border border-gray-100 flex flex-col h-full">
            <h2 className="font-bold text-lg mb-3 flex items-center gap-2">
              <Calendar className="w-4 h-4 text-blue-500" /> Upcoming Bookings
            </h2>
            {upcoming.length === 0 ? (
                  ...
                  <li key={booking.id} className="border-b pb-2">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold">
                        {booking.service} with {booking.professional}
                      </span>
                      <span
                    ...
                    <button className="mt-2 text-xs bg-blue-600 text-white rounded px-3 py-1 transition-transform duration-200 hover:scale-105 hover:shadow-md hover:bg-blue-700">
                      Reschedule / Cancel
                    </button>
                  </li>
                ))}
          ...
          <div className="bg-white rounded-2xl shadow p-4 sm:p-6 border border-gray-100 flex flex-col h-full">
            <h2 className="font-bold text-lg mb-3 flex items-center gap-2">
              <Clock className="w-4 h-4 text-blue-500" /> Booking History
            </h2>
            {history.length === 0 ? (
                ...
                {history.map((booking) => (
                  <li key={booking.id} className="border-b pb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="ml-auto px-2 py-0.5 rounded text-xs font-semibold bg-green-100 text-green-800">
                        {booking.status}
                      </span>
                    </div>
                    <div className="text-xs text-gray-600 flex items-center gap-2 mt-1">
                    ...
                    <div className="flex gap-2 mt-2">
                      <button
                        className="text-xs bg-green-600 text-white rounded px-3 py-1 transition-transform duration-200 hover:scale-105 hover:shadow-md hover:bg-green-700"
                        onClick={() => {
          ...
          <div className="bg-white rounded-2xl shadow p-4 sm:p-6 border border-gray-100 flex flex-col justify-between h-full">
            <div>
              ...
              <div className="flex items-center gap-3 mb-2">
                <span className="inline-block w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 font-bold text-lg border">
                  {userProfile.name[0]}
                </span>
              ...
              <button
                ...
                className="flex items-center gap-1 text-blue-600 text-xs transition-transform duration-200 hover:scale-105 hover:shadow-md hover:underline"
              >
            ...
            <div className="mt-6">
              ...
              <div className="flex flex-col gap-2">
                <Link
                  href="/book"
                  className="bg-blue-600 text-white px-4 py-2 rounded font-semibold text-center transition-transform duration-200 hover:scale-105 hover:shadow-md hover:bg-blue-700"
                >
                  Book New Service
                </Link>
                <button
                  ...
                  className="bg-gray-100 text-blue-600 px-4 py-2 rounded font-semibold text-center transition-transform duration-200 hover:scale-105 hover:shadow-md hover:bg-blue-50"
                >
                  Contact Support
                </button>
                <Link
            ...
            <table className="min-w-full border border-gray-200 rounded text-sm">
              ...
              <tbody>
                {payments.map((payment) => (
                  <tr key={payment.id}>
                    <td className="border-t px-4 py-2">{payment.service}</td>
                    <td className="border-t px-4 py-2">{payment.date}</td>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-sm animate-fadeIn" style={{ animation: 'fadeIn 0.2s' }}>
            ...
            {supportSuccess ? (
              ...
              <form onSubmit={handleSupportSubmit}>
                <label className="block text-xs font-semibold mb-1" htmlFor="support-message">
                  Message
                </label>
                <textarea
                  id="support-message"
                ...
                <div className="flex gap-2 justify-end mt-4">
                  <button
                    type="button"
                    className="px-4 py-2 rounded bg-gray-100 text-gray-700 transition-transform duration-200 hover:scale-105 hover:shadow-md hover:bg-gray-200"
                    onClick={() => {
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2 rounded bg-blue-600 text-white transition-transform duration-200 hover:scale-105 hover:shadow-md hover:bg-blue-700"
      ...
      {showFeedbackModal && feedbackBooking && (
        <div
          ...
          onClick={(event) => {
            if (event.target === feedbackModalRef.current) {
              setShowFeedbackModal(false);
            }
          }}
        >
          <div
            ...
            className="bg-white rounded-xl shadow-lg p-6 w-full max-w-sm animate-fadeIn"
            style={{ animation: 'fadeIn 0.2s' }}
          >
            <h3 className="text-lg font-bold mb-4">Leave Feedback</h3>
            <div className="mb-2 font-semibold">
              {feedbackBooking.service} with {feedbackBooking.professional}
            </div>
            <div className="flex gap-1 mb-4">
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  ...
                  className={star <= feedbackStars ? 'text-yellow-400' : 'text-gray-300'}
                >
                  <svg
      ...
      {showProfileModal && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/30"
          onClick={(event) => {
            if (event.target === profileModalRef.current) {
        ...
        >
          ...
          >
            ...
            >
              <div className="mb-3">
                <label className="block text-xs font-semibold mb-1" htmlFor="profile-name">
                  Name
                </label>
                <input
                  id="profile-name"
              ...
              <div className="mb-3">
                <label className="block text-xs font-semibold mb-1" htmlFor="profile-email">
                  Email
                </label>
                <input
                  id="profile-email"
              ...
              <div className="mb-3">
                <label className="block text-xs font-semibold mb-1" htmlFor="profile-phone">
                  Phone
                </label>
                <input
                  id="profile-phone"
                  ...
                  onChange={(event) => setEditProfile({ ...editProfile, phone: event.target.value })}
                />
              </div>
              <div className="flex gap-2 justify-end mt-4">
                <button
