...
const optionalField = z.string().max(120).optional().or(z.literal(''));

const bookingSchema = z.object({
  service: z.string().min(1, 'Select a service'),
  summary: z.string().min(10, 'Tell us a bit more (at least 10 characters)').max(300, 'Keep summary under 300 characters'),
  address: z.string().min(5, 'Enter an address'),
  eircode: z.string().min(3, 'Enter your Eircode'),
  preferredDate: optionalField,
  preferredTime: optionalField,
...
function BookPageContent(): JSX.Element {
  ...
  useEffect(() => {
    ...
    setValue('service', prefilledService, { shouldValidate: true });
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      ...
      (Object.entries(values) as Array<[keyof BookingValues, string]>).forEach(([key, value]) => {
        const trimmed = value.trim();
        if ((key === 'preferredDate' || key === 'preferredTime') && trimmed.length === 0) {
          return;
        }
    ...
    return (
      <div className="glass-surface rounded-3xl border border-white/10 bg-white/10 p-10 text-center">
        ...
        <p className="mt-3 text-white/70">
          Your reference number is <span className="font-semibold text-white">{successRef}</span>. We’ve emailed details to
          {session?.user?.email ? ` ${session.user.email}` : ' your inbox'}.
        </p>
  ...
  return (
    <form className="space-y-6" onSubmit={onSubmit}>
        ...
        <div className="glass-surface rounded-3xl border border-white/10 bg-white/10 p-8">
          ...
          <div className="mt-6 grid gap-4">
            <label className="flex flex-col gap-2 text-sm text-white/70">
              ...
              {errors.service && <span className="text-xs text-fx-amber">{errors.service.message}</span>}
            </label>
            <label className="flex flex-col gap-2 text-sm text-white/70">
              Problem summary
              <textarea
              ...
              {errors.summary && <span className="text-xs text-fx-amber">{errors.summary.message}</span>}
            </label>
          </div>
        </div>
      )}
          ...
          <div className="mt-6 grid gap-4">
            <label className="flex flex-col gap-2 text-sm text-white/70">
              ...
              <input
                ...
                className="rounded-xl border border-white/10 bg-white/10 px-3 py-3 text-white outline-none focus:border-fx-primary"
              />
              {errors.address && <span className="text-xs text-fx-amber">{errors.address.message}</span>}
            </label>
            <label className="flex flex-col gap-2 text-sm text-white/70">
              Eircode
              <input
                ...
                className="rounded-xl border border-white/10 bg-white/10 px-3 py-3 text-white outline-none focus:border-fx-primary"
              />
              {errors.eircode && <span className="text-xs text-fx-amber">{errors.eircode.message}</span>}
            </label>
      ...
      {submissionError && <p className="text-sm text-fx-amber">{submissionError}</p>}
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2 text-sm text-white/60">
          {steps.map((label, index) => (
            <span
              key={label}
              className={`flex h-3 w-3 rounded-full ${index <= step ? 'bg-fx-primary' : 'bg-white/20'}`}
            />
          ))}
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          {step === steps.length - 1 && (
            <FxButton type="submit" loading={isSubmitting} disabled={isSubmitting} aria-label="Submit booking">
              Submit booking
            </FxButton>
          )}
        </div>
...
function BookingPageShell(): JSX.Element {
  return (
    <div className="fx-container space-y-8 py-16">
      <div className="max-w-2xl space-y-3">
        <h1 className="text-4xl font-semibold text-white">Book a service</h1>
        <p className="text-white/60">
          Complete the steps below to create a new FixEasy booking. We’ll confirm within minutes and keep you updated every
          step.
        </p>
      </div>
      <BookPageContent />
